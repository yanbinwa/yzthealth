	...
	if (!empty($postStr)){
		...
		if($type=='location'){
			...
			if($wid == '75829'){
				...
				$dataCity = $addressComponent['result']['addressComponent']['city'];
				/*log::warn('----$wid=75829---wid='.$dataCity);*/
				$chinaarea = new Model('chinaarea');
				$chinaarea->find(array('name'=>$dataCity));
				$lres = $m->where(array('wid'=>$wid,'l_shi'=>$chinaarea->id))->list_all();
				if (empty($lres)) {
					$lres = $m->where(array('wid'=>$wid))->list_all()->order('id desc')->limit('0,9');
				}
		...
		if($type=='event'){
			if('SCAN'==$postObj->Event)
			{
				  ...
				  if($wid=='6769607')  //肯德基
				  {
					  $om = new Model('dz_md6769607_gz_log');
					   //记录关注
						  $om->wid = $wid;
						  $om->mdid = $sceneid1;
				  ...
				  if($sceneid1=='3571' && $wid=='6768305' )
				  {
						$mdm1 = new Model('dz_md6768305');
						$mdm1->find($sceneid1);
						if($mdm1->has_id())
						{
						       $res = new Model('res');
				  ...
				  if($wid=='6769607' && in_array($sceneid1,$mdarr))
				  {
					 $res = new Model('res');
					 $res->find('312966');
					 response_arts($res,$postObj); 
			...
			if('SCAN'==$postObj->Event)
			{
				  ...
				  $sceneid1=str_replace("qrcode@","",$postObj->EventKey); 
				  if($sceneid1=='3898' && $wid=='6769607' )
				  {
						$mdm1 = new Model('dz_md6769607a');
						$mdm1->find($sceneid1);
						if($mdm1->has_id())
						{
						       //肯德基扫描领卡券
				  ...
				  //11.29肯德基扫描就推图文  262346
				  if($wid=='6769607' && $sceneid1!='3898')
				  {
					   $res = new Model('res');
					   $res->find('267345');
					   response_arts($res,$postObj);      
				  ...
				  if($wid=='6769607' && ($sceneid1=='3925' || $sceneid1=='3926' || $sceneid1=='3927') )
				  {
					   $res = new Model('res');
					   $res->find('309205');
					   response_arts($res,$postObj);      
			...
			if('subscribe'==$postObj->Event){
				//判断场景ID 进行处理
				// 社区渠道定制
				if (isset($postObj->EventKey) && $wid == '6784481'){
					$info = substr($postObj->EventKey,8);
					if(strlen($info) > 3){
						$qid = substr($info,0,3);
						$wxid = substr($info,3);
						if($qid){
							...
							$m = new Model('makeewm_fans');
							$m = $m->find(array('wxid'=>$postObj->FromUserName));
							if(!$m->has_id()){
								$m->wid = $wid;
								$m->wxid = $postObj->FromUserName;
								$m->qid = $qid;
								$m->createtime = date("Y-m-d",time());
								$m->nc = $jsoninfo['nickname'];
								$m->pic = $jsoninfo['headimgurl'];
								$m->status = 1;
								$m->t_wxid = $wxid;
								$m->save();
								$qd_user = new Model('dk_user');
								$qd_user = $qd_user->find(array('wid'=>'6784481','wxid'=>$postObj->FromUserName));
								if($qd_user->shequ_id){
									$qd_user->shequ_id = $qid;
									$qd_user->save();
								}

							}elseif($m->has_id() && $m->status == 2){
								$m->wid = $wid;
								$m->wxid = $postObj->FromUserName;
								$m->qid = $qid;
								$m->createtime = date("Y-m-d",time());
						...
						$dk_user = new Model('dk_user');
						$dk_user = $dk_user->find(array('wid'=>'6784481','wxid'=>$wxid));
						if($dk_user->id){
							$makeewm_set = new Model('makeewm_ewm_set');
							$makeewm_set = $makeewm_set->find(array('wid'=>'6784481'));
							$dk_user->integral = $dk_user->integral + $makeewm_set->tui_jf;
							$dk_user->save();
							$jf_log = new Model('dk_jf_log');
							$jf_log->wid = '6784481';
							$jf_log->uid = $dk_user->id;
							$jf_log->content = '专属推广 赠送积分';
							$jf_log->jf = $makeewm_set->tui_jf;
							$jf_log->creat_time = time();
							$jf_log->model = 'add';
							$jf_log->save();
						}
					}else{
				...
				if(strpos($postObj->EventKey,"qrcode@")){
					...
					if($sceneid!='')
					{
					  ...
					  if($wid=='6769607' && in_array($sceneid,$mdarr))
					  {
						 $res = new Model('res');
						 $res->find('312966');
						 response_arts($res,$postObj); 
					  ...
					  if($wid=='6769607' && ($sceneid=='3925' || $sceneid=='3926' || $sceneid=='3927') )
					  {
						   $res = new Model('res');
						   $res->find('309205');
						   response_arts($res,$postObj);      
			...
			if('subscribe'==$postObj->Event){
				...
				if($fa->has_id()){
					if(empty($sceneid) && $wid=='6769607')
					{
					   $fa->typ=0;
					}
					if($wid=='6768305' && $sceneid=='1103567') //必胜客
					{
					   $fa->resource_id = '250474';
					   $fa->typ = 1;
					}
					
					if($wid=='6769607' && $sceneid=='3898') //肯德基扫描领卡券
					{
					   $fa->resource_id = '267345';
					   $fa->typ = 1;
					}
					
					if($wid=='6780859' && $sceneid=='3911')
					{
					     $fa->resource_id = '313304';
					     $fa->typ = 1;
					...
					if($fa->typ=='0')
					{
						...
						$fa_content = $fa->content;
						$fa_content = htmlspecialchars_decode($fa_content);
						$fa_content = str_replace('<br>', "\n", $fa_content);
						$fa_content = str_replace("\r\n", "\n", $fa_content);


						response_text($fa_content,$postObj);
 					}
					else
					{
					    //图文
						...
						if(!empty($sceneid) && $res->id=='249442')  //肯德基定制  
						{
							Cache::set($postObj->FromUserName."kdj",$sceneid,10000);
							$mdm = new Model('dz_md6769607a');
						    $mdm->find($sceneid);
						    //替换标题
							$conarr = json_decode($res->con);
							$aa = str_replace("华联店", "$mdm->name", "$conarr->tit");
							$conarr->tit = $aa;
							$res->con =  json_encode($conarr);
						}
						response_arts($res,$postObj); 
					}
				}
			...
			if('unsubscribe'==$postObj->Event){
				...
				if($wid == 6784481){
					...
					if($m->has_id()){
						$m->status = 2;
						$m->endtime = date("Y-m-d",time());
						$m->save();
						if($m->t_wxid){
							$dk_user = new Model('dk_user');
							$dk_user = $dk_user->find(array('wid'=>'6784481','wxid'=>$m->t_wxid));
							if($dk_user->id){
								$makeewm_set = new Model('makeewm_ewm_set');
								$makeewm_set = $makeewm_set->find(array('wid'=>'6784481'));
								$jian = $dk_user->integral - $makeewm_set->tui_jf;
								if($jian <= 0){
									$jian = 0;
								}
								$dk_user->integral = $jian;
								$dk_user->save();
								$jf_log = new Model('dk_jf_log');
								$jf_log->wid = '6784481';
								$jf_log->uid = $dk_user->id;
								$jf_log->content = '被推广人取消关注 扣积分';
								$jf_log->jf = $makeewm_set->tui_jf;
								$jf_log->creat_time = time();
								$jf_log->model = 'reduc';
								$jf_log->save();
						...
						$duser = new Model('dk_user');
						$duser = $duser->find(array('wid'=>'6784481','wxid'=>$postObj->FromUserName));
						if($duser->has_id()){
							$duser->shequ_id = '';
							$duser->tui_ewm = '';
							$duser->save();
...
//								response_text('头像上传成功', $postObj);
		...
		}elseif($keyword!=''){			//text回复
				
			if($keyword=='宝宝'){
				response_text2("升级成功666",$postObj);
				//记住账户的uuid
				$pub = new Model('pubs');
...
function onthewall($qlog,$keyword,$postObj,$isfirst=false){
	...
	if(trim($qlog->nc)==''){
		if($isfirst){
			...
			if($qlog1->has_id())
			{
			   response_text('该昵称已被注册过', $postObj);
			}
			else
			{
			    $qlog->nc = $keyword;
	...
	if(trim($qlog->pic)==''){
		response_text('请上传图片作为您的头像；如需退出请回复“0”', $postObj);
		return;
	}else{
		//上墙
...
//	微信墙
	...
	if ($keywordModel->has_id()) {
	 	...
	 	}elseif($keywordModel->type =="micro_jiduihuan_active_list"){  // 
	 		$model = new Model($keywordModel->type);
	 		$model->find(array('wid'=>$wid,'id'=>$keywordModel->pid));
 			if($model->has_id()){
				$url = Conf::$http_path.'jiduihuan/activev1-'.$model->id.'.html?wxid='.$postObj->FromUserName.'&wid='.$wid;
				response_one($model->title,$model->pic,$model->jianjie,$url,$postObj);
				return;
	 	...
	 	}elseif($keywordModel->type =="micro_ltcw_set"){  // 微商城订制，乐托宠物
			$model = new Model($keywordModel->type);
			$model->find(array('wid'=>$wid,'id'=>$keywordModel->pid));
			if($model->has_id()){
				$url = Conf::$http_path.'wsc_ltcw/index.html?wid='.$wid;
				response_one($model->name,$model->pic,$model->ms,$url,$postObj);
				return;
		...
		elseif($keywordModel->type =="z02_set"){  // 微商城
			...
			if($model->has_id()){
	 		...
	 		$model = new Model($keywordModel->type);
	 		$model->find(array('wid'=>$wid,'id'=>$keywordModel->pid));
			log::warn('+++++++++微喜帖+++++++++++++++wid='.$wid.'id='.$keywordModel->pid);
	 			if($model->has_id()){
	 		...
	 		$model->find(array('wid'=>$wid,'id'=>$keywordModel->pid));
	 			if($model->has_id()){
					...
					if($model->typ=='queqiao' || $model->typ=='shenjingmao' || $model->typ=='nuannan' || $model->typ=='yetaitu')
					{
						...
						$url = "www.weixinguanjia.cn/".$model->typ.'/index.html?&wxid='.$postObj->FromUserName.'&wid='.$wid;
					}
					else
					{
...
function getMediaByMediaId($wid,$mediaId){
	$ACCESS_TOKEN = getAccessTokenByWid($wid);
		log::warn('ACCESS_TOKEN=mpapi='.$ACCESS_TOKEN.'wid='.$wid.'$mediaId='.$mediaId);
...
function response_text($txt,$postObj){
	...
	$res  = sprintf($textTpl,$postObj->FromUserName,$postObj->ToUserName,time(),$txt);
	echo $res;
	exit();
}

function response_text2($txt,$postObj){
	$textTpl = "<xml>
	               <ToUserName><![CDATA[%s]]></ToUserName>
	               <FromUserName><![CDATA[%s]]></FromUserName>
	               <CreateTime>%s</CreateTime>
...
function response_oneart($r,$rid,$postObj){
	...
	if(strpos($r->ourl,'mp.weixin.qq.com') == false){
		$r->ourl = $r->ourl.'#mp.weixin.qq.com';
	}
	$npic = checkUrl($r->pic);

	$item=sprintf($subitem, $r->tit, $r->des, $npic, $r->ourl);
...
function response_morearts($res,$rid,$postObj){
	...
	foreach ($res as $r){
		...
		$item.=sprintf($subitem, $r->tit, $npic, $r->ourl);
	}
	$resstr = str_replace('ITEM', $item, $resstr);
	echo $resstr;
}
...
function response_one($tit,$pic,$des,$url,$postObj){
	...
	$purl = checkUrl($pic);
	$item=sprintf($subitem, $tit, $des, $purl, $url);
	$resstr = str_replace('ITEM', $item, $resstr);
	echo $resstr;
}
...
//回复多图文
function response_more($res,$postObj,$t=0){
	global $wid;
	$fromUsername = $postObj->FromUserName;
	$toUsername = $postObj->ToUserName;
	$textTpl = "<xml>
	...
	foreach ($res as $r){
		if(strpos($r['url'],'mp.weixin.qq.com') == false){
			$r['url'] = $r['url'].'#mp.weixin.qq.com';
		}
		$npic = checkUrl($r['pic']);

		if($t==1)
		$item.=sprintf($subitem, $r['tit'],'', $r['url']);
		else
		$item.=sprintf($subitem, $r['tit'],$npic, $r['url']);
	}
	$resstr = str_replace('ITEM', $item, $resstr);
	echo $resstr;
}
