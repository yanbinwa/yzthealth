...
if('del'==Request::get(1)){
	...
	if(is_array($id))
	{
		...
		if('recycle'==Request::get('type')){
			//进入回收站
			$sql = 'UPDATE z02_sprop SET is_del = "1" WHERE wid ="'.$wid.'" and unid="'.$unid.'" and id in ('.$id_str.')';
			$db->query($sql);
			exit('成功移入回收站！');
		}else{
			//彻底删除
			$sql = 'DELETE FROM z02_sprop WHERE wid ="'.$wid.'" and unid="'.$unid.'"  and id in ('.$id_str.')';
			$db->query($sql);
			exit('删除成功！');
		}
	...
	if(is_array($id))
	{
		...
		//数据恢复
		$sql = 'UPDATE z02_sprop SET is_del = "0" WHERE wid ="'.$wid.'" and unid="'.$unid.'" and id in ('.$id_str.')';
		$db->query($sql);
		exit('成功恢复！');
...
// 查找对应属性值
	...
	if('recycle'==Request::get(1)){
		$m->where(array('wid'=>$wid,'is_del'=>1,'unid'=>$unid));
	}else{
		$m->where(array('wid'=>$wid,'is_del'=>0,'unid'=>$unid));
	}
	$p = new Pagination();
	$res = $p->model_list($m);
}