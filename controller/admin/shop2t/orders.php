...
if(Request::get('trade_no')){
	...
	$where .=" and o.trade_no='".$trade_no."'";
}
if(Request::get('shopname')){
	$shopname = Request::get('shopname');
	$where .=" and u.shopname like '%".$shopname."%'";
}
if(Request::get('rename')){
	$rename = Request::get('rename');
	$where .= " and o.full_name like '%".$rename."%'";
}
if(Request::get('time')){
	...
	$end = strtotime($arr[1]);
	$where .= " and o.updated_at between $star and $end";
}
$db = DB::get_db();
$sql = "select o.id,o.total,o.amount,o.trade_no,o.created_at,o.pay_status,o.end_time,o.ll_time,o.status,o.telephone,u.shopname,o.full_name from orders as o ";
$sql.= "left join lgc_supplier_user as u on u.id = o.unid ";
$sql.= "where ".$where." order by o.id desc ";

$p = new Pagination();
$rs = $p->sql_list($sql); 

$zcount = $orderModel->where(array('pay_status'=>1,'status'=>1))->count();
$orderModel_arr = $orderModel->where(array('pay_status'=>1,'status'=>1))->list_all_array();
$zamount = 0;
$shiji = 0;
