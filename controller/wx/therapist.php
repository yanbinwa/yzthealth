...
if (!empty($wid) && !empty($wxid)) {
	...
	/*评论总数*/
	$orders_arr = $orders->where(array('status'=>1,'pay_status'=>1,'remark_status'=>1,'unid'=>$unid))->list_all_array();
	$remark_count = count($orders_arr);
	/*好评总数*/
	$orders_good_remark = $orders->where(array('status'=>1,'pay_status'=>1,'remark_status'=>1,'remark_type'=>1,'unid'=>$unid))->list_all_array();
	$good_remark_count = count($orders_good_remark);
	/*好评度*/	
	$favorable_rate = floor(($good_remark_count/$remark_count)*100);
	/*订单总数*/
	$ordersed_arr = $orders->where(array('status'=>1,'pay_status'=>1,'unid'=>$unid))->list_all_array();
	...
	$sproduct_array = $sproduct->where(array('unid'=>$unid,'is_ll'=>1,'sh_status'=>1,'status'=>0))->order('id')->list_all();
    //总平台添加商品
    ...
    if (!Session::get('fansid') ||$f->wid != $wid){
        //高级接口获取用户信息
        $Wxapi->get_authorize_url(conf::$http_path.'wx/therapist.html');
    }	
}
