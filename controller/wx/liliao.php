...
if (!empty($wid) && !empty($wxid)) {
    ...
    if($fans->id){
        ...
        if ($chinaarea->id) {
            ...
            $sup_arr = $supplier->where(array('is_lls'=>0,'status'=>1,'shopname@~'=>$searchword))->list_all_array(); 
            if (count($sup_arr)>0) {
                $where.=" and shopname like '%".$searchword."%' "; 
            }else{
        ...
        if (!empty($shop_arr)) {
            ...
            if ($page<=$count) {
                ...
                for ($i=$page; $i < $count; $i++) { 
                    $sql = "SELECT id,shopname,jianjie,address,dz,jd,wd,order_number from lgc_supplier_user WHERE id='".$v."'";
                    ...
                    $supplier_array[$j]['shopname'] = $sup_arr[0]['shopname'];
            ...
            $sql = "SELECT id,shopname,jianjie,address,dz,jd,wd,order_number from lgc_supplier_user WHERE ".$where." order by id desc limit $page,10";
            ...
            foreach($supplier_array as $k=>$v){
                ...
                $supplier_array[$k]['shopname'] = mb_substr($v['shopname'],0,10,'utf-8');
            }            
        }
