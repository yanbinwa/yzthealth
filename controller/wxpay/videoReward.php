...
$artice_list->find(array('id'=>$vid));

$rewards = new Model('rewards');

$out_trade_no = "WXSPDS_".time();

$rewards->aid = $vid;
$rewards->wxid = $wxid;
$rewards->rewardprice = $artice_list->reward;
$rewards->trade_no =$out_trade_no;
$rewards->status = 0;
$rewards->createtime = time();
$rewards->save();
...
include_once("/home/wwwroot/wxpay/WxPayHelperv33.php");
	...
	$unifiedOrder->setParameter("body",'学院视频打赏');//商品描述
	//自定义订单号，此处仅作举例

	//$timeStamp = time();
	$unifiedOrder->setParameter("out_trade_no",$out_trade_no);//商户订单号
	$unifiedOrder->setParameter("total_fee",intval(floatval($artice_list->reward)*100));//总金额
	$unifiedOrder->setParameter("notify_url",Conf::$http_path.'wx/rewardReturn-'.$vid.'-'.$wxid.'.html');//通知地址
			...
			WeixinJSBridge.invoke(
				...
				function(res){
					...
					if(res.err_msg=='get_brand_wcpay_request:ok'){
						location.href='<?php echo Conf::$http_path;?>wx/mediaVideo-<?php echo $vid;?>.html';		
					}else{
						location.href='<?php echo Conf::$http_path;?>wx/mediaList1-108.html';
					}
