...
Builder.prototype.init = function () {
    ...
    var opt = this.option;
    var $target = $("#" + opt.id);
    $target.find("#micro_plugmenulisttype").change(function () {
...
Builder.prototype.getResult = function () {
    var opt = this.option;
    var $target = $("#" + opt.id);
    var module = $target.find("#micro_plugmenulisttype").val();
    var data = null;
...
Builder.prototype._loadBaidu_map = function () {
    ...
    if (data) {
    	...
    	var jd = $("#micro_plugmenulistjd").val();
    	var wd = $("#micro_plugmenulistwd").val();
    ...
    function doit(point) {
        ...
        if (point) {
            ...
            document.getElementById('micro_plugmenulistwd').value = point.lat;
            document.getElementById('micro_plugmenulistjd').value = point.lng;
            map.setCenter(point);
            ...
            myGeo.getLocation(point, function (result) {
                if (result) {
                    document.getElementById('micro_plugmenulistplace').value = result.address;
                }
            });
            ...
            marker2.addEventListener("dragend", function (e) {
                document.getElementById('micro_plugmenulistwd').value = e.point.lat;
                document.getElementById('micro_plugmenulistjd').value = e.point.lng;
                myGeo.getLocation(new BMap.Point(e.point.lng, e.point.lat), function (result) {
                    if (result) {
                        $('micro_plugmenulistplace').value = result.address;
            ...
            map.addEventListener("dragend", function showInfo() {
                ...
                myGeo.getLocation(new BMap.Point(cp.lng, cp.lat), function (result) {
                    if (result) {
                        document.getElementById('micro_plugmenulistplace').value = result.address;
                        document.getElementById('micro_plugmenulistwd').value = cp.lat;
                        document.getElementById('micro_plugmenulistjd').value = cp.lng;
                        //	window.external.setaddress(result.address);//setarrea(result.address);//
                        //alert(point.lng + "  ddd " + point.lat);
    ...
    function loadmap() {
        ...
        var city = document.getElementById('micro_plugmenulistplace').value;
    ...
    function setarrea(address, city) {
        $('micro_plugmenulistplace').value = address;
        //$('city').value=city;
        window.setTimeout(function () {
            loadmap();
        }, 2000);
    ...
    $("#micro_plugmenulistplace").change(function () { loadmap(); })
...
window.ICON = function () {
   ...
   $("#ico_hot").html(ul.format(s));
    ...
    $(".icon_list li").live("click", function () {
        ...
        $("#micro_plugmenulisticon").val($thisclass);
        $(".icons-cont").hide();
    });
