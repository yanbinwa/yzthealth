...
function tp1(res)
{
     ...
     (function(){
        dds_HTML = iTemplate.makeList(TPL, res, function(k,v){
             return {
...
function tp2(dishes)
{
	var tpehtml='';
    var checkHtml = '';
    var TPL = '<dl id="list_{id}" dunitname="{unit}" dsubcount="1" dishid="{id}" dname="{name}" dtaste="" ddescribe="{desc}" dprice="{price}" dishot="{tag_name}" dspecialprice="{discount_price}" disspecial="1" onclick="selectOne(this, {id}, {price});">\
            <dt><h3>{name}</h3></dt>\
            <dd>\
        ...
        tpehtml = iTemplate.makeList(TPL, dishes, function(k,v){
...
function Menutpl(data)
{
	...
	var myorder = document.getElementById("myorder");
	 myorder.innerHTML =  iTemplate.makeList(TPL,data, function(k, v) {
	 if (v.discount_price) {
	     v.price = v.discount_price;
	 }
	 totalPrice += parseFloat(v.price) * parseInt(v.selected_count);
	 totalsum = parseInt(totalsum) + parseInt(v.selected_count);
	 v.selected_count
	 return {
	...
	document.getElementById("countnums").innerHTML = totalsum;
	$("#countnums").attr("nums",totalsum);
	document.getElementById("num").innerHTML = totalsum;
}
function intetpl(res)
{
	   window.res.randIndex ++;
       window.res.randIndex = window.res.randIndex%window.res.data.length;
	   var TPL = '<article><h2>{note}</h2>\
	         <dl dunitname="{unit}" dsubcount="{dishes_count}"  dname="{name}" dtaste="" ddescribe="{note}" dprice="{yprice}" dishot="{discount_name}" dpicture="{pic}" dspecialprice="{discount_price}" >\
	             <dt><h3>{name}</h3></dt>\
	             <dd>\
	             ...
	             <dd><em>{price}元/{unit}</em> </dd>\
	             <dd class="dpFen">×{dishes_count}</dd>\
	         </dl>\
        </article>';
		var price = 0;
		var zprice = 0;
		var intetpl = iTemplate.makeList(TPL, window.res.data[window.res.randIndex].dishes, function(k,v){
			v.yprice = v.price;
			...
			var fprice = 0;
			if(v.discount_price){
				fprice = ((v.discount_price)*(v.dishes_count)).toFixed(2);
			}
			else
			{
				fprice = ((v.price)*(v.dishes_count)).toFixed(2);
			}
		    zprice += parseFloat(fprice);
		});
		document.getElementById("priceCount").innerHTML = (zprice).toFixed(2) + "元";
		return intetpl;
}

function checkordertpl(data)
{
	   var TPL = '<li>\
         <span class="dishName">{name}</span>\
         <i>{price}元/{unit}</i>\
         <section class="bbox" dishname="{name}">\
         ×{selected_count}\
         </section>\
         </li>';
		var myorder = document.getElementById("myorder");
