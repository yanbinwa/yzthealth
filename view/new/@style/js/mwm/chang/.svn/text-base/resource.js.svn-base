            ...
            if (result.success) {
                ...
                if (data.length > 0) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        cont.push('<td style="display:none;">' + item.start_time + '~' + item.end_time + '</td>');
                        cont.push('</tr>')
                    }
...
Builder.prototype._loadAct = function () {
    ...
    var select = $target.find("#act");
    var tt = select.find("option:selected").data("list");

    if (tt) {
        $("#r_activity div.margin-top").show();
    //var top = 0;
    $.post("/common/getactivity", {
        aid: data.aid,
        type: t
    }, function (result) {
        if (result.success) {
        ...
        $("#r_activity div.margin-top").hide();
